...
public class DruidDataSource extends DruidAbstractDataSource
    ...
    // store
    private volatile MpmcArrayQueue<DruidConnectionHolder> connections;
    ...
    private void setPoolPreparedStatements0(boolean value) {
        ...
        if (!value) {
            lock.writeLock().lock();
            try {
                DruidConnectionHolder[] holders = new DruidConnectionHolder[maxActive];
                DruidConnectionHolder connection;
                int cnt = 0;
                while ((connection = connections.poll()) != null) {
                    poolingCount.decrementAndGet();
                    holders[cnt++] = connection;
    ...
    public void setMaxActive(int maxActive) {
        ...
        try {
            int capacity = 2;
            while (capacity < maxActive) {
                capacity <<= 2;
            }
            MpmcArrayQueue<DruidConnectionHolder> newConnections = new MpmcArrayQueue<>(capacity);
            DruidConnectionHolder connection;
            int cnt = 0;
            while ((connection = connections.poll()) != null) {
                if (cnt < maxActive) {
    ...
    public boolean discardConnection(Connection conn) {
        ...
        if (activeCount.get() + poolingCount.get() < minIdle) {
            requestConnectionSignal();
    ...
    private DruidPooledConnection getConnectionInternal(long maxWait) throws SQLException {
        ...
        while (true) {
            try {
                ...
                if (holder != null) {
                    if (holder.discard) {
                        // decrease activeCount as it has been increased by create connection thread.
                        activeCount.decrementAndGet();
                        holder = null;
    ...
    public void close() {
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("{dataSource-" + this.getID() + "} close operation interrupted. " + e.getStackTrace());
            }
            return;
        }
        ...
        try {
            ...
            if (connections != null) {
                ...
                while ((connHolder = connections.poll()) != null) {
                    poolingCount.decrementAndGet();
    ...
    private boolean mpscQueueAdd(MpscArrayQueue<DruidConnectionRequest> mpscQueue, DruidConnectionRequest req) {
        try {
            return mpscQueue.add(req);
    ...
    boolean putLast(DruidConnectionHolder holder, long lastActiveTimeMillis) throws InterruptedException {
        holder.lastActiveTimeMillis = lastActiveTimeMillis;

        // busy offering for pursuing performance.
        while (enable && !Thread.interrupted()) {
            if (connections.offer(holder)) {
            ...
            } else if (failFast && isFailContinuous()) {
                if (holder.active) {
                    activeCount.decrementAndGet();
                    holder.active = false;
                }
    ...
    private void incActiveCountAndDecPoolingCount() throws InterruptedException {
        this.lock.readLock().lockInterruptibly();
        try {
    ...
    DruidConnectionHolder takeLast() throws InterruptedException, SQLException {
        ...
        try {
            ...
            // busy waiting for pursuing performance.
            boolean addOk = false;
            ...
            if (last != null) {
                ...
                while (enable
                        && !(isInterrupted = Thread.interrupted())
                        && !req.isStopping()) {
                    last = req.getDruidConnectionHolder();
                    if (last != null) {
                        break;
                    }
                    requestConnectionSignal();
            ...
            if (last != null) {
                ...
                req.setInvalid(true);
                threadRequestInvalidMap.put(req, System.currentTimeMillis());
            }
        ...
        try {
            ...
            if (last != null) {
                ...
                while (enable
                    ...
                    requestConnectionSignal();
                    req.getDoneLatch().await(maxWaitNanos - waitNanos, TimeUnit.NANOSECONDS);
                    waitNanos = TimeUnit.MILLISECONDS.toNanos(System.currentTimeMillis() - startTime);
                }
            }
    ...
    public DruidDataSourceStatValue getStatValueAndReset() {
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug("{dataSource-" + this.getID() + "} interrupted. " + e.getStackTrace());
            }
            return null;
        }
        try {
    ...
    private boolean put(DruidConnectionHolder holder, DruidConnectionRequest connReq) {
        // keep lock only for unit tests.
        try {
            lock.writeLock().lockInterruptibly();
        } catch (InterruptedException e) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("{dataSource-" + this.getID() + "} put connection to pool failed. " + e.getStackTrace());
            }
            return false;
        }
        try {
            ...
            if (activeCount.get() + poolingCount.get() >= maxActive) {
                return false;
            }

            if (connReq == null || connReq.isInvalid()) {
                ...
                int peak = activePeak.get();
                if (count > peak) {
                    activePeak.compareAndSet(peak, count);
                    activePeakTime = System.currentTimeMillis();
                }
                connReq.setDruidConnectionHolder(holder);
    ...
    public class CreateConnectionThread extends Thread {
        ...
        public void run() {
            ...
            try {
                if (initTask && initialSize > 0) {
                    while (!closing && !closed
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (LOG.isDebugEnabled()) {
                                    LOG.debug("{dataSource-" + DruidDataSource.this.getID()
                                            + "} interrupted." + e.getStackTrace());
                                }
                                break;
                            }
                            try {
                    ...
                    if (poolingCount.get() < initialSize) {
                        exiting = true;
                        // close data source priors to sending stopping notice.
                        if (!closing && !closed) {
            ...
            while (!closing && !closed && !Thread.currentThread().isInterrupted()) {
                ...
                if (connReq != null && (candidate = connections.poll()) != null) {
                    // update activeCount and poolingCount when poll the connection from the pool.
                    try {
                        incActiveCountAndDecPoolingCount();
                    } catch (InterruptedException e) {
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("{dataSource-" + DruidDataSource.this.getID() + "} interrupted. " + e.getStackTrace());
                        }
                        break;
                    }
                ...
                try {
                    allCount = activeCount.get() + poolingCount.get();
                } finally {
                    lock.writeLock().unlock();
                }
        ...
        private void setMpscQueueStoppingNotice() {
    ...
    public void shrink(boolean checkTime, boolean keepAlive) {
        ...
        try {
            ...
            if (keepAlive && poolingCount.get() + activeCount.get() < minIdle) {
                needFill = true;
            }
        } finally {
            lock.writeLock().unlock();
            ...
            if (this.isPoolPreparedStatements()) {
                ...
                for (int i = 0; i < cnt; ++i) {
                    if (conn != null) {
                        ...
                        buf.append("\n\t{\n\tID:");
                        buf.append(System.identityHashCode(holders[i].getConnection()));
                        PreparedStatementPool pool = holders[i].getStatementPool();
    ...
    private boolean isFillable(int toCount) {
        lock.readLock().lock();
        try {
            int currentCount = this.poolingCount.get() + this.activeCount.get();
    ...
    public boolean isFull() {
        lock.readLock().lock();
        try {
            return this.poolingCount.get() + this.activeCount.get() >= this.maxActive;
        } finally {
            lock.readLock().unlock();
        }
    }

    private void requestConnectionSignal() {
        LockSupport.unpark(createConnectionThread);
    }
